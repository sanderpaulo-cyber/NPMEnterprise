...
1. Com a **API em execução**, execute `npm run auth:test-login` — se devolver **200**, o servidor aceita `admin` / `ChangeMeAdmin2026!` e o problema é só no browser (URL, cookie, proxy). Se devolver **401**, a instância da API na porta do `.env` usa **outra base** que a do `npm run auth:debug-users` (ex.: Docker com Postgres interno vs `DATABASE_URL` local no host).
2. Na **mesma máquina onde corre a API**, execute `npm run auth:debug-users` — confirma se existe `admin`, se tem hash e se a password `ChangeMeAdmin2026!` bate certo com a base a que o `.env` aponta. Se aqui falhar, rode `npm run auth:reset` nessa máquina.
3. **Password predefinida** (primeiro utilizador / bootstrap): `ChangeMeAdmin2026!` — respeite maiúsculas e o `!` no fim. O utilizador é **`admin`** (minúsculas no login).
4. Se já existiam utilizadores na base antes do bootstrap, a password pode ser outra. Redefina com: `npm run auth:reset` (ou `npm run auth:create-user -- admin NovaPasswordForte123`).
5. Confirme `AUTH_ENABLED=true` e `AUTH_JWT_SECRET` (≥16 caracteres) no `.env` e reinicie a API.
6. Com a API actualizada, após uma tentativa de login falhada os **logs da API** indicam o motivo (utilizador inexistente nesta BD, sem password local, hash inválido, ou password errada).
7. **LDAP**: se `AUTH_LDAP_URL` estiver preenchido mas o servidor LDAP não estiver acessível, o login local deve funcionar na mesma (a API ignora erros de LDAP após tentar a password local). Se ainda falhar, comente temporariamente as variáveis `AUTH_LDAP_*` e reinicie.
8. **Dashboard em HTTPS** (`WEB_HTTPS=1`): o proxy Vite envia `X-Forwarded-Proto` para a API definir o cookie `ns_session` corretamente. O login e a sessão usam sempre o proxy `/api` no **mesmo host** que a página (ignoram URL directa na Conexão). Em **Configurações → Conexão**, deixe a URL base **vazia** para não partir o cookie entre `localhost` e `127.0.0.1`.
9. Em último caso, defina `AUTH_COOKIE_SECURE=false` no `.env` da API (só desenvolvimento) se o cookie não for gravado.
